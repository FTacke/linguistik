...
Hinweis: Die produktiven Quelldateien liegen weiter unter `docs/assets/audiofiles/`. Die Skripte selbst leben jetzt unter `tools/audio/`; Reports und normalisierte Vergleichsdateien sind lokale Laufartefakte und werden nicht publiziert.
...
- Generierte Reports unter `tools/audio/reports/` spiegeln den Endstand nach der produktiven Uebernahme wider, bleiben aber ausserhalb des publizierten Inhalts.
...
- `repo-docs/design-system/admonition-reference.md`
- `docs/aussprache.md`
...
## Audio-Tooling unter `tools/audio/`

Dauerhaft versioniert sind nur die beiden Skripte:

- `tools/audio/audio_audit.py`
- `tools/audio/audio_normalize.py`

Die Verzeichnisse `tools/audio/reports/` und `tools/audio/normalized/` werden bei Bedarf erzeugt und koennen jederzeit neu erstellt werden.
...
Die normalisierten Vergleichsdateien bleiben parallel unter `tools/audio/normalized/` erhalten.
...
- schreibt Vergleichsdateien unter `tools/audio/normalized/`
...
```powershell
c:/dev/linguistik.hispanistica/.venv/Scripts/python.exe tools/audio/audio_audit.py
```

### Voll-Normalisierung mit produktiver Uebernahme ausfuehren

```powershell
c:/dev/linguistik.hispanistica/.venv/Scripts/python.exe tools/audio/audio_normalize.py --overwrite --promote
```
...
```powershell
c:/dev/linguistik.hispanistica/.venv/Scripts/python.exe tools/audio/audio_normalize.py --dry-run
```

Teilmenge pruefen:

```powershell
c:/dev/linguistik.hispanistica/.venv/Scripts/python.exe tools/audio/audio_normalize.py --filter corapan/URY --limit 5
```

Explizite Tool-Pfade setzen, falls `PATH` oder Alias-Aufloesung nicht greift:

```powershell
c:/dev/linguistik.hispanistica/.venv/Scripts/python.exe tools/audio/audio_audit.py --ffmpeg C:/pfad/zu/ffmpeg.exe --ffprobe C:/pfad/zu/ffprobe.exe
```

## Reports und QS-Grundlage

Die zentrale Arbeitsbasis liegt unter `tools/audio/reports/`.
